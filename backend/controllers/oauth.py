    ...
    try:
        ...
        try:
            ...
            if state_data.get("reregister"):
                ...
                logger.info("[oauth] reregister provider=%s email=%s ip=%s", provider, provider_email, ip)
            else:
                user = users.get_or_create_oauth_user(oauth_user)
                if user.get("status") == users.DELETED:
                    user_id = str(user.get("id"))
                    logger.info("[oauth] deleted_user provider=%s email=%s ip=%s redirect reregister", provider, provider_email, ip)
                    auth.record_login_attempt(
                        db=db,
                        user_id=user_id,
                        provider=provider,
            ...
            logger.warning("[oauth] provider=%s email=%s ip=%s error=account_inactive", provider, provider_email, ip)
            return redirect_after_login_failure()
